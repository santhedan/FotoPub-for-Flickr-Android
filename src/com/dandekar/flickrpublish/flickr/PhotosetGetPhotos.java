...
public class PhotosetGetPhotos extends BaseRequest {
	...
	public String getUrl() {
		return String.format("%s?%s&%s&%s&%s&%s&%s&%s&%s&%s&%s&%s&%s&%s", this.url, this.nojsoncallback, this.format,
				this.consumerKey, this.authToken, this.method, this.userId, this.signature, this.nonce, this.timeStamp,
				this.signatureMethod, this.version, this.extra, this.photosetId);
	}
